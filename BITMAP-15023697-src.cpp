...
void breadthFirstSearch(int i,int j){
    ...
    list<pair<int,int> >VertexQueue;
    list<pair<int,int> >::iterator itr=VertexQueue.begin();
     
    VertexQueue.push_back(make_pair(i,j));

    while(!VertexQueue.empty()){

          itr=VertexQueue.begin();
          ...
          if(i+1<=n){
                 ...
                 if(level[i][j]+1<level[i+1][j]){
                        ...
                        VertexQueue.push_back(make_pair(i+1,j));  
          ...
          if(j+1<=m){
                  ...
                  if(level[i][j]+1<level[i][j+1]){
                       ...
                       VertexQueue.push_back(make_pair(i,j+1));    
